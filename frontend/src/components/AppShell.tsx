  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(194,65,12,0.10),_transparent_30%),linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)]">
      <div className="mx-auto grid min-h-screen max-w-[1600px] grid-cols-1 gap-6 px-4 py-6 lg:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="rounded-[28px] border border-slate-800 bg-slate-950 px-5 py-6 text-slate-100 shadow-panel">
          ...
          <button
            type="button"
            onClick={() => void logout()}
            className="mt-8 w-full rounded-xl border border-slate-700 px-4 py-3 text-sm text-slate-100 transition hover:bg-slate-800"
          >
            退出登录
          </button>
        </aside>
        ...
        <main className="space-y-6">
          <header className="rounded-[28px] border border-slate-200 bg-white px-6 py-5 shadow-panel">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="text-sm font-medium text-slate-700">账号：{user.username}</div>
            </div>
          </header>
